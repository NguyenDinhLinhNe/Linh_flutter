...
dart create -t server-shelf . --force
```
sẽ tạo một dự án Dart với mẫu -t, template là server-shelf trong thư mục hiện tại . và tham số --force cho biết sẽ tạo dự án cho dù thư mục gốc đã tồn tại (mặc định là sẽ tạo mới thư mục).
...
Chỉnh sửa file backend/bin/server.dart:
...
```
Debug backend và kiểm thử với Postman

Thêm Middleware xử lý CORS cho backend
